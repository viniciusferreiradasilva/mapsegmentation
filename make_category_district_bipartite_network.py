...
categories_file = args.output_dir + args.input_file.split('/')[-1].split('.')[0] + "_categories.csv"
pd.DataFrame({"id":list(categories.values()), "category": list(categories.keys())}).to_csv(categories_file, index=None)
...
vertices_list = list(categories.values()) + list(np.array(districts['cluster_id']) + len(categories))
...
edge_list = [(y, x + len(categories)) for x in list(districts['cluster_id']) for y in districts_categories_count[x]]
...
network_output_file = args.output_dir + args.input_file.split('/')[-1].split('.')[0] + '_' + clustering_algorithms[
    args.clustering_algorithm].__name__ + '_' + str(number_of_categories) + '_' + str(number_of_districts) + '.ncol'
...
embedding_file = args.output_dir + args.input_file.split('/')[-1].split('.')[0] + '_' + clustering_algorithms[
        args.clustering_algorithm].__name__ + '_' + str(number_of_categories) + '_' + str(number_of_districts) +\
                 '_district_emb.txt'
...
csv_file = args.output_dir + args.input_file.split('/')[-1].split('.')[0] + '_' + clustering_algorithms[
        args.clustering_algorithm].__name__ + '_' + str(number_of_categories) + '_' + str(number_of_districts) + '.csv'
